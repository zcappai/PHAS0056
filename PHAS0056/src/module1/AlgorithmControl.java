...
public class AlgorithmControl {
	...
	public static void main(String[] args) {
		...
		//Calculating total number of loops for loop runtime (millisecond)
		int totalLoops = ac.timer(10000, 1000); //10 seconds, 1000 loops
		int totalLoops1 = ac.timer(10000, 50000); //10 seconds, 50000 loops
		System.out.println("\nTotal Number of Loops:"+totalLoops);
		System.out.println("\nTotal Number of Loops:"+totalLoops1);
	}
	...
	public void loop() {
		...
		for(int counter = 1; counter<=max; counter++) { //"counter++" increases value by 1
...
//			x++; //Counter increased by 1 per loop
			...
			//"If" statement prints total loops for every 100 loops
//			if(x % 100 == 0) {
...
//	}
	...
	public int timer(long maxTime, int loopSteps) {
		long start = System.currentTimeMillis(); //Assigns current time to "start"
		long end = start + maxTime; //"end" time is "maxTime" after "start" time
		int x = 0; //Initial condition for counter
		
		//"While" loop increases value of counter by 1 per loop during "maxTime" interval
		while(System.currentTimeMillis() < end) {
			x++; //Counter increased by 1 per loop
