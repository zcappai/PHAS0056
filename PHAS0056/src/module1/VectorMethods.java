...
public class VectorMethods {
	public static void main(String[] args) {
		...
		//Variables for 3-dimensional vectors used to calculate angle between vectors
		...
		System.out.println("2b)Since the 2nd vector is a zero vector with magnitude zero, it has no direction. And since angle is calculated between directions, the angle is undefined.");
	}
	//DEFINITION OF METHOD TO FIND MAGNITUDE OF VECTOR (3D)
	public double magnitude(double x1, double y1, double z1) {
		//Magnitude of vector calculated by squaring each element, adding them together and then square rooting
		double magn = Math.sqrt(Math.pow(x1, 2)+Math.pow(y1, 2)+Math.pow(z1, 2));
		return magn; //Returns magnitude
	}
	//DEFINITION OF METHOD TO FIND SCALAR PRODUCT OF 2 VECTORS (3D)
	...
	public double angle(double x1, double y1, double z1, double x2, double y2, double z2) {
		//Below 'dotProduct' method called and scalar product of 2 vectors calculated
		double dot1 = dotProduct(x1, y1, z1, x2, y2, z2);
		//Below 'magnitude' method is called and magnitude of 2 vectors calculated
		...
		return Math.toDegrees(Math.acos(cosx)); //Returns angle by inverse cosine of 'cosx', then converts to degrees
	}
}
